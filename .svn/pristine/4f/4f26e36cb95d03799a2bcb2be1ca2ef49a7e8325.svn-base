function setCookieChat(c_name, value, exdays) {
    var exdate = new Date();
...
var EnableChatSound = getCookieChat("EnableChatSound");
EnableChatSound = EnableChatSound != null ? (EnableChatSound == "true") : true;

var chatServerUrl = "https://s1.banxehoi.com"; //local http://chat.bxh.com
var senderId = '';
var senderName = '';
var productId = 0;
var chatPK = '';
var senderGender = false;
...
BDSChat = new function () {
    ...
    var handlerUrl = "/Chat/ProcessRequest";
    var cookieRoomName = "CHAT_ROOM";
    var roomHistories = null;
    var classTimeChat = "chat-time-";
    var defaultAvatar = "/images/customer.png";
    var defaultProductImage = "/images/no-photo.jpg";
    var defaultSupportAvatar = "/Content/Chat/Styles/images/support-avatar.png";
    var defaultMainSupportAvatar = "/Content/Chat/Styles/images/support-avatar-default.png";
    var chatSoundUrl = "/content/Chat/scripts/ChatSound.mp3";
    ...
    var emoticonOpt = {
        handle: '#etoggle',
        dir: 'https://img1.banxehoi.com/chat_images/emoticons/',
        label: 'On Emoticons',
    ...
    window.onbeforeunload = function () {
        setCookieChat("Signalr_ConnectionKey", "", 1);
        if (chatRooms > 0) {
    ...
    this.attachEvents = function () {
        ...
        senderGender = senderGender != null ? senderGender : 0;
        ...
        if ($.connection != null) {
            ...
            $.connection.hub.qs = "__ui=" + encodeURIComponent(senderId) + "&__gd=" + senderGender + "&__un=" + encodeURIComponent(senderName) + (chatPK != null && chatPK != '' && isAdmin ? "&__pk=" + encodeURIComponent(chatPK) : '');
    ...
    this.registerEvents = function () {
        ...
        if ($.connection != null) {
            $.connection.chatServer.client.initiateChatUI = function (chatRoom, isOnline, guestInfo, totalUnread) {
                ...
                if (currentUrl == '/member/quan-ly-chat') {
                    return;
                }
                ...
                if (rooms[chatRoom.chatRoomId] != undefined) {
                    ...
                    $('#divChatWithBDS').hide();
                    var e = $(strNewChatroomTemplate).tmpl(chatRoom);
                    var c = $(strNewChatHeader).tmpl(chatRoom);
                    ...
                    if (!isAdmin) {
                        ...
                        if (chatRoom.productId > 0) {
                            ...
                            if (BDSChat.loadSupportDetail(divChatMessage, chatRoom, isOnline)) {
                                ...
                                c.find(".user-status").after($("<span />").addClass("link-chat-bds-header-detail"));
                    ...
                    if (!isUserMgr) {
                        rooms[chatRoom.chatRoomId].popup = e.chatPopup({
                            ...
                            "title": c,
                            "sex": BDSChat.getGender(chatRoom.chatUsers),
                    ...
                    $('#newmessage-' + chatRoom.chatRoomId).focus(function () {
                        ...
                        if (rooms[chatRoom.chatRoomId] != undefined && rooms[chatRoom.chatRoomId].popup != null) {
                            rooms[chatRoom.chatRoomId].popup.addUnreadMessageExternal(chatRoom.chatRoomId, 0);
                        }

                        // Update read message for room by roomId and sender id 
                        if ((roomHistories == null || roomHistories.length == 0) && (!rooms[chatRoom.chatRoomId].isRead)) {
                            rooms[chatRoom.chatRoomId].isRead = true;
                            $this.chatServer.server.updateReadMessage(chatRoom.chatRoomId);
                        }

                        var index = document.title.indexOf(')');
                        var pageTitle = index > 0 ? document.title.substring(index + 2) : document.title;
                    ...
                    if (!isAdmin) {
                        ...
                        if (rooms[chatRoom.chatRoomId].isChatSound) {
                            ...
                            chatPopup.find(".divIsRoomSound").css("background-image", 'url(/content/Chat/styles/images/chat-checked-icon.png)');
                        } else {
                            chatPopup.find(".divIsRoomSound input").val("0");
                            chatPopup.find(".divIsRoomSound").css("background-image", 'url(/content/Chat/styles/images/chat-unchecked-icon.png)');
                        }

                        if (rooms[chatRoom.chatRoomId].isSendEmail) {
                            chatPopup.find(".divIsSendEmail input").val("1");
                            chatPopup.find(".divIsSendEmail").css("background-image", 'url(/content/Chat/styles/images/chat-checked-icon.png)');
                        } else {
                            chatPopup.find(".divIsSendEmail input").val("0");
                            chatPopup.find(".divIsSendEmail").css("background-image", 'url(/content/Chat/styles/images/chat-unchecked-icon.png)');
                        }
                        ...
                        $("#chatPopup-" + chatRoom.chatRoomId + " .divIsRoomSound").click(function () {
                            ...
                            if (input.val() == "1") {
                                ...
                                input.val("0");
                                $(this).css("background-image", 'url(/content/Chat/styles/images/chat-unchecked-icon.png)');
                            } else {
                                input.val("1");
                                //rooms[chatRoom.chatRoomId].isChatSound = true;
                                $(this).css("background-image", 'url(/content/Chat/styles/images/chat-checked-icon.png)');
                            }
                            //BDSChat.setCookie();
                        });
                        ...
                        $("#chatPopup-" + chatRoom.chatRoomId + " a.btnsave").click(function () {
                            ...
                            if (userChatInfo.IsGuest) {
                                ...
                                $.ajax({
                                    url: handlerUrl,
                                    data: { action: 'updatechatuser', email: email, phone: phoneNumber },
                        ...
                        $("#chatPopup-" + chatRoom.chatRoomId + " a.btnclear").click(function () {
                            ...
                            if (rooms[chatRoom.chatRoomId].isChatSound) {
                                ...
                                cPopup.find(".divIsRoomSound").css("background-image", 'url(/content/Chat/styles/images/chat-checked-icon.png)');
                            } else {
                                cPopup.find(".divIsRoomSound input").val("0");
                                cPopup.find(".divIsRoomSound").css("background-image", 'url(/content/Chat/styles/images/chat-unchecked-icon.png)');
                            }

                            if (rooms[chatRoom.chatRoomId].isSendEmail) {
                                cPopup.find(".divIsSendEmail input").val("1");
                                cPopup.find(".divIsSendEmail").css("background-image", 'url(/content/Chat/styles/images/chat-checked-icon.png)');
                            } else {
                                cPopup.find(".divIsSendEmail input").val("0");
                                cPopup.find(".divIsSendEmail").css("background-image", 'url(/content/Chat/styles/images/chat-unchecked-icon.png)');
                            }
                    ...
                    if (!isOnline && guestInfo != null) {
                        ...
                        var offlineGuestMess = 'Liên hệ với khách hàng qua ';
                        if (guestInfo.Phone != null && guestInfo.Phone != '') {
                            offlineGuestMess += 'SĐT: ' + guestInfo.Phone;
                        }
                        if (guestInfo.Email != null && guestInfo.Email != '') {
                            offlineGuestMess += '\nEmail: ' + guestInfo.Email;
                        }
                        newMsg.val(offlineGuestMess);
            ...
            $.connection.chatServer.client.receiveChatMessage = function (chatMessage, chatRoom, isOnline) {
                ...
                rooms[chatRoom.chatRoomId].isRead = false;
                ...
                if (senderId == chatMessage.senderId) {
                    ...
                    e = $(strNewMessageTemplateRight).tmpl(chatMessage).appendTo(chatRoomMessages);
                } else {
                    e = $(strNewMessageTemplateLeft).tmpl(chatMessage).appendTo(chatRoomMessages);
            ...
            $.connection.chatServer.client.receiveErrorMessage = function (errorMessage) {
                if (errorMessage.chatRoomId != "") {
                    ...
                    var divError = $(strChatMessageErrorTemplate).tmpl(errorMessage);
            ...
            $.connection.chatServer.client.receiveSupporter = function (chatUsers) {
                ...
                for (var i = 0; i < chatUsers.length; i++) {
                    ...
                    $.ajax({
                        url: handlerUrl,
                        type: 'POST',
                        data: { action: 'getavatar', fileid: (chatUsers[i].Avatar == null ? "" : chatUsers[i].Avatar) },
                        async: false,
                        success: function (data) {
                    ...
                    if (!chatUsers[i].IsOnline) {
                        avatarUrl = chatUsers[i].Order < 10 ? '/Content/Chat/Styles/images/support-avatar-offline.png' : '/Content/Chat/Styles/images/support-partner-avatar.png';
                    }
                    else {
                    ...
                    if (chatUsers[i].Order == 12) {
                        item = $(strChatTopBankTemplate).tmpl({
                            UserId: chatUsers[i].UserId,
                            FullName: chatUsers[i].FullName,
                            AvatarUrl: avatarUrl,
                            IsOnline: (chatUsers[i].IsOnline ? 'online' : '')
                        });
                    } else if (chatUsers[i].Order >= 10) {
                        item = $(strChatLstSupportPartnerTemplate).tmpl({
                            UserId: chatUsers[i].UserId,
                            FullName: chatUsers[i].FullName,
                            AvatarUrl: avatarUrl,
                            IsOnline: (chatUsers[i].IsOnline ? 'online' : '')
                        ...
                        item = $(strChatLstSupportTemplate).tmpl({
                            UserId: chatUsers[i].UserId,
                            FullName: chatUsers[i].FullName,
                            AvatarUrl: avatarUrl,
                            IsOnline: (chatUsers[i].IsOnline ? 'online' : '')
    ...
    this.LogOutSingR = function (isAdmin) {
        $this.chatServer.server.logOutSingR(userChatInfo.UserId, isAdmin);
    }
    ...
    this.endChat = function (chatRoomId) {
        ...
        $this.chatServer.server.endChat(chatMessage).fail(function (ex) {
            ...
            console.log("End chat failure: " + ex.toString());
        });
    };

    this.initiateChat = function (toUserId, productId) {
        if ($this.chatServer == null) {
            console.log("Problem in connecting to Chat Server. Please Contact Administrator!");
        ...
        $this.chatServer.server.initiateChat(productId, toUserId).fail(function (ex) {
            ...
            console.log("Initiate chat failure: " + ex.toString());
        });
    };
                ...
                if (rooms[key].toUserId != undefined) {
                    if (rooms[key].toUserId == user.UserId) {
                        ...
                        if (isOnline) {
                            ...
                            statusIcon.addClass("offline-status");
        ...
        for (var i = 0; i < supportItems.length; i++) {
            if ($(supportItems[i]).attr("uid") == user.UserId) {
                if (isOnline) {
                    ...
                    $(supportItems[i]).find(".support-item-img img").attr("src", "/Content/Chat/Styles/images/support-avatar.png");
                } else {
                    $(supportItems[i]).find(".support-item-status").removeClass("online");
                    $(supportItems[i]).find(".support-item-img img").attr("src", "/Content/Chat/Styles/images/support-avatar-offline.png");
                }
                break;
            }
        }
    ...
    this.getAvatar = function (chatMessage, chatRoom, messaege) {
        ...
        //    rooms[chatRoom.chatRoomId].avatarUrl = "http://file4.batdongsan.com.vn/images/support-avatar.png";
        //    BDSChat.updateAvatar("http://file4.batdongsan.com.vn/images/support-avatar.png", chatRoom.chatRoomId, messaege);
        //    return;
        //}
        ...
        if (rooms[chatRoom.chatRoomId].avatarUrl != null && chatMessage != null) {
            ...
            $.ajax({
                ...
                async: false,
                type: 'POST',
                data: { action: 'getavatar', fileid: (fileId == null ? "" : fileId) },
                success: function (data) {
    ...
    this.loadProductDetail = function (divChatMessage, chatRoom, isOnline) {
        $.ajax({
            ...
            type: 'POST',
            dataType: 'json',
            data: { action: 'getAuto', autoId: chatRoom.productId, chatRoomId: chatRoom.chatRoomId },
            async: false,
            success: function (data) {
                if (data != null) {
                    var productDiv = $(strChatProductDetailsTemplate).tmpl(data);
    ...
    this.displayWelComeMessage = function (divChatMessage, chatRoom, message) {
        ...
        e = $(strNewMessageTemplateLeft).tmpl(chatMessage);
    ...
    this.loadSupportDetail = function (divChatMessage, chatRoom, isOnline) {
        ...
        for (var i = 0; i < chatRoom.chatUsers.length; i++) {
            if (chatRoom.chatUsers[i].UserId == userId) {
                if (chatRoom.chatUsers[i].IsStaff) {
                    var supportU = $(strChatCallerBdsTemplate).tmpl({ userId: userId, FullName: chatRoom.chatUsers[i].FullName, OnlineText: (isOnline ? "Sẵn sàng hỗ trợ" : "Đang offline") });
                    divChatMessage.before(supportU);
                    $.ajax({
                        url: handlerUrl,
                        type: 'POST',
                        dataType: 'json',
                        data: { action: 'getavatar', fileid: (chatRoom.chatUsers[i].Avatar == null ? "" : chatRoom.chatUsers[i].Avatar) },
                        async: false,
                        success: function (data) {
                            var avatarUrl = defaultMainSupportAvatar;
                            if (data.avatarUrl.length > 0) {
                                avatarUrl = data.avatarUrl;
                            }
                            ...
                            supportAvatar.error(function () {
                                rooms[chatRoom.chatRoomId].avatarUrl = defaultMainSupportAvatar;
                                supportAvatar.attr("src", defaultMainSupportAvatar);
                            });
                        }
                    });

                    if (isOnline) {
                        this.displayWelComeMessage(divChatMessage, chatRoom, "Rất vui vì đã ghé thăm Banxehoi.com. Bạn có cần tư vấn hoặc giải đáp thắc mắc gì, hãy chat với chúng tôi!");
                    } else {
                        this.displayWelComeMessage(divChatMessage, chatRoom, "Rất vui vì bạn đã ghé thăm Banxehoi.com. Hiện CSKH đang không online, vui lòng để lại lời nhắn cho chúng tôi!");
                    }
                    return true;
                }
            }
    ...
    this.addChatArea = function () {
        ...
        if (isAdmin) {
            ...
            $("body").append(divChatContainer);
            divChatContainer.append($(strChatContainerTemplate).tmpl());
        ...
        if (roomStr != undefined && roomStr.length > 0) {
            ...
            if (roomArr.length > 1 && roomArr[0] == senderId) {
                for (var i = 1; i < roomArr.length; i++) {
                    var roomInfoArr = roomArr[i].replace(/,/g, "#").split('#');
    ...
    this.setCookie = function () {
        ...
        if (roomIds.length > 0) {
            ...
            //console.log(cookieRoomName + " ----- " + cookieVal);
        } else {
    ...
    this.loadChatHistoryMessage = function (chatMessage, chatRoom, currentTime) {
        ...
        if (senderId == chatMessage.senderId) {
            if (messageWelcome.length != 0) {
                e = $(strNewMessageTemplateRight).tmpl(chatMessage);
                messageWelcome.after(e);
            } else {
                e = $(strNewMessageTemplateRight).tmpl(chatMessage).prependTo(chatRoomMessages);
            }
        } else {
            if (messageWelcome.length != 0) {
                e = $(strNewMessageTemplateLeft).tmpl(chatMessage);
                ...
                e = $(strNewMessageTemplateLeft).tmpl(chatMessage).prependTo(chatRoomMessages);
            }
    ...
    this.chatWithProductOwner = function (userId, fullName, productId) {
        ...
        if (senderId == "") {
            ...
            var chatReg = $(strChatRegTempalte).tmpl({ userId: userId, fullName: fullName, productId: productId, bdsClass: bdsClass, bdsIconClass: bdsIconClass });

            if ($('.ban_scroll').is(":visible")) {
                $(chatReg).css('right', 170);
            }
            if (productId > 0) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    dataType: 'json',
                    data: { action: 'getAuto', autoId: productId },
                    async: false,
                    success: function (data) {
                        if (data.success) {
                            var product = $(strChatProductDetailsTemplate).tmpl(data.product);
            ...
            chatReg.find("#dangnhap_chat").on("click", function () {
                $('#dangnhap_chat').fancybox({
                    fitToView: false
                });
            });
            ...
            chatReg.find("#btnChatClear").click(function () {
                $("#txtChatPhoneNumber").val(""); //support - item - status
                $("#txtChatEmail").val("");
                $("#txtChatName").val("");
                ...
                $('#regMessage').html('');
                chatReg.find(".chat-reg-save-info span").css("background-image", 'url(/content/Chat/styles/images/chat-checked-icon.png)');
            });
            ...
            $("#txtChatPhoneNumber").numeric();
            var changeCaptcha = function () { $('#img_CAPTCHA_RESULT_123456').attr('src', '/chat/GenerateCaptcha?t=' + (new Date().getTime())); };
            chatReg.find("#btnStartChat").click(function () {
                if ($('#txtChatPhoneNumber').val().trim() == '') {
                    $('#regMessage').html('Bạn cần nhập số điện thoại.');
                    $('#txtChatPhoneNumber').focus();
                    changeCaptcha();
                    return false;
                }
                ...
                if (/[~`!#$%\^&*+=\-\[\]\\';,/{}|\\":<>\?]/g.test($('#txtChatName').val().trim())) {
                    $('#regMessage').html('Tên không được chứa ký tự đặc biệt!');
                    $('#txtChatName').focus();
                    changeCaptcha();
                    return false;
                }

                if ($('#txtChatEmail').val().trim() == '') {
                    $('#regMessage').html('Bạn cần nhập email.');
                    $('#txtChatEmail').focus();
                    changeCaptcha();
                    return false;
                }

                if (!BDSChat.checkEmail($('#txtChatEmail').val().trim())) {
                    $('#regMessage').html('Bạn cần nhập đúng định dạng email.');
                    $('#txtChatEmail').focus();
                    changeCaptcha();
                    return false;
                }
                //if ($('input[name=rdGender]:checked').val() == null) {
                //    $('#regMessage').html('Bạn cần chọn giới tính.');
                //    //  $('input[name=rdGender]').focus();
                //    changeCaptcha();
                //    return false;
                //}
                ...
                if (/^[a-zA-Z0-9- ]*$/.test($('#txtChatCapchar').val().trim()) == false) {
                    $('#regMessage').html('Mã an toàn không được chứa ký tự đặc biệt!');
                    $('#txtChatCapchar').focus();
                    changeCaptcha();
                    return false;
                }
                ...
                $.ajax({
                    url: handlerUrl,
                    type: 'POST',
                    data: { action: 'regchatuser', name: $('#txtChatName').val(), phone: $('#txtChatPhoneNumber').val(), email: $('#txtChatEmail').val(), captcha: $("#txtChatCapchar").val(), sex: $('input[name=rdGender]:checked').val(), isSaveInfo: $("#chkChatSaveInfo").val() },
                    dataType: 'json',
                    success: function (data) {
                        if (data.Success) {
            ...
            chatReg.find(".chat-reg-save-info span").click(function () {
                if ($("#chkChatSaveInfo").val() == "1") {
                    ...
                    chatReg.find(".chat-reg-save-info span").css("background-image", 'url(/content/Chat/styles/images/chat-unchecked-icon.png)');
                } else {
                    $("#chkChatSaveInfo").val("1");
                    chatReg.find(".chat-reg-save-info span").css("background-image", 'url(/content/Chat/styles/images/chat-checked-icon.png)');
                }
    ...
    this.playSound = function () {
        ...
        var popsound = new Audio('http://banxehoi.com/content/Chat/scripts/ChatSound.mp3');
        popsound.load();
        popsound.play();
    };
    ...
    this.loadUnreadMessageForMenu = function () {
        $.ajax({
            url: handlerUrl,
            type: 'POST',
            data: { action: 'getinboxunreadroom' },
            success: function (data) {

                var quickInboxUnread = $('#chat-quick-inbox-icon .mnu-chat-icon-unread');
                if (data != null && data.data != null && data.data.length > 0 && quickInboxUnread.length > 0) {
                    if (data.totalUnread > 0) {
                        quickInboxUnread.text(data.totalUnread);
                    ...
                    if (inboxContent.length > 0) {
                        ...
                        for (var i = 0; i < data.data.length; i++) {
                            html += "<li id='quick-inbox-item-" + data.data[i].Room.RoomId + "' onclick=\"BDSChat.quickInboxItemClick(this,'" + data.data[i].User.UserId + "','" + data.data[i].Room.ProductId + "', '" + data.data[i].Room.RoomId + "')\">";
                            html += "<span class='quick-inbox-item-title'>" + data.data[i].User.FullName + "</span><br/>";
                            html += "<span class='quick-inbox-item-lastMessage'>" + BDSChat.subWords(data.data[i].ChatMessage.messageText, 42) + "</span><br/>";
                            html += "<span class='quick-inbox-item-time'>" + BDSChat.convertTime(data.data[i].ChatMessage.timestamp, (new Date()).getTime()) + "</span>";
                            if (data.data[i].IsUnRead) {
                                html += "<span class='quick-inbox-item-new'><img src=\"/Styles/images/icon-new3.png\" class=\"new\"></span>";
                            }
                            html += "</li>";
                        }
                        ...
                        $('#chat-quick-inbox-icon').click(function () {
                            if ($('#pnlQuickBoxChat:visible').length > 0) {
                                $('#pnlQuickBoxChat').hide();

                            } else {
                                $('#pnlQuickBoxChat').show();
                            }
                        });
                    ...
                    $('#chat-quick-inbox-icon').click(function () {
                        ...
                        location.href = '/member/quan-ly-chat';
                    });
                }
            }
        });
    ...
    this.quickInboxItemClick = function (obj, userId, productId, chatRoomId) {
        BDSChat.initiateChat(userId, productId);
        $this.chatServer.server.updateReadMessage(chatRoomId);
        // $(obj).remove();
        var count = $('#chat-quick-inbox-icon .mnu-chat-icon-unread').html() - 1;
        if (count > 0) {
            $('#chat-quick-inbox-icon .mnu-chat-icon-unread').html(count);
        }
        else {
            $('#chat-quick-inbox-icon .mnu-chat-icon-unread').html("");
            $('#chat-quick-inbox-icon .mnu-chat-icon-unread').hide();
        }
        $(".chat-quick-inbox-content").removeClass("active");
        $(obj).find('.quick-inbox-item-new').remove();
    }
    ...
    this.checkPhoneNumber = function (strPhone) {
        ...
        if (strPhone.substring(0, 1) != "0") { return false; }
        if (strPhone != '' && (strPhone.length < 10 || strPhone.length > 11))
        { return false; }
        var phoneNumberPattern = /^[(]{0,1}[0-9]{3,5}[)]{0,1}[-\s.]{0,1}[0-9]{3}[-\s.]{0,1}[0-9]{3}$/;
        if (!phoneNumberPattern.test(strPhone.replace(/ /g, ''))) {
            return false
        }
        else {
            return true;
...
$(window).scroll(function () {
    $('.emoticon-avatar').each(function () {
        $('.emoticons').css('top', $(this).offset().top - $('.emoticons').outerHeight() + "px");
    })
});
$(window).scroll(function () {
    $('.emoticon-avatar').each(function () {
...
$(document).click(function () {
    if (this.id != 'chat-quick-inbox-icon') {
        $('#pnlQuickBoxChat').hide()
    }
});
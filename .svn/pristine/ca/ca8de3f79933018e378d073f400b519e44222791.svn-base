...
(function ($) {
    ...
    var chatPopup = function (options) {
        ...
        var init = function () {
            ...
            var divChatPopup = $(strChatPopupTemplate).tmpl({ id: opts.id, fullName: opts.fullName })
	            .css({
	                "width": opts.width,
	                "bottom": '0',
	                "right": right,
	            ...
	            .appendTo(chatArea);
            $(strChatPopupHeaderTemplate).tmpl().appendTo(divChatPopup);
        ...
        var showRoomById = function (roomId) {
            ...
            //$('#newmessage-' + roomId).val("Nhập nội dung và nhấn &quot;enter&quot; để chat");
        };
        ...
        var closeChatRoom = function (roomId) {
            if ($('.collapse-room-list li').length <= 1) {
                $('#divChatWithBDS').show();
            }
        ...
        var initCollapseRoom = function () {
            $(strChatCollapseRoomTemplate).tmpl().appendTo($(".list-chat-room"));
        ...
        var addCollapseItem = function (roomId, fullName, productTitle) {
            var item = $(strChatCollapseRoomItemTemplate).tmpl({ roomId: roomId, fullName: (productTitle == '' ? fullName : fullName + " - " + productTitle) });
        ...
        this.addUnreadMessageExternal = function (roomId, count) {
            ...
            if (unreadItem.length > 0) {
                if (count == 0) {
                    ...
                    $('.collapse-chat-room').find('.unread-count').each(function () {

                        if ($(this).html() != "") {

                            totalUnreadMessage += parseInt($(this).html().match(/\d+/)[0]);
                        }
                    });
                    console.log(totalUnreadMessage);
                    document.title = "(" + totalUnreadMessage + ") " + pageTitle;
                }
            }
            updateRoomUnread();
        };
